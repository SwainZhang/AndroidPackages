...
/**
 * 通过Http协议上传文件，适用于2M大小以内的文件，如果是视频等大文件应采用其他方法
 * 如果上传文本内容，可直接使用Post提交，该类主要是用于上传小文件
 */
public class SocketHttpRequester {
	...
	public static String post(String path, Map<String, String> params, FormFile[] files) throws Exception{
		...
		URL url = new URL(path);
		//得到端口号，如果是-1 就用80，否则取得端口号
		int port = url.getPort()==-1 ? 80 : url.getPort();
		//创建一个Socket，链接到服务器的某一个端口，如80端口
		Socket socket;
		if (true/*NetConstManager.getNetConst().isDebug()*/) {
			/*Proxy proxy = new Proxy(Proxy.Type.SOCKS, new InetSocketAddress(
					NetConst.PROXY_HOST, 80));
			socket = new Socket(proxy);
			SocketAddress remoteAddr = new InetSocketAddress(InetAddress.getByName(url.getHost()),port);
			socket.connect(remoteAddr);*/
			//socket = new Socket(InetAddress.getByName(url.getHost()),port);
			socket = new Socket(/*NetConstManager.getNetConst().getProxyHost()*/"", 80);
		} else {
			socket = new Socket(InetAddress.getByName(url.getHost()),port);
		}
...
//        conn.setRequestMethod("POST"); // Post方式
		...
		for(FormFile uploadFile : files){
			...
			//分割线前面多了两个--符号，加进来，都是从http协议参考过来的
			fileEntity.append("--");
			...
			//先判断文件有没有输入流，以便确定调用哪个构造方法（该JavaBean里有两个构造方法）
			if(uploadFile.getInStream()!=null){
		...
		while(!msg.equals("0")){
			Log.e("", "outStream readLine" + System.currentTimeMillis() + msg);
			sb.append(msg);
			msg = reader.readLine();
		}
	 ...
	 * @param file 上传文件
	 */
	public static String post(String path, Map<String, String> params, FormFile file,boolean isUpload) throws Exception{
		if(isUpload){
